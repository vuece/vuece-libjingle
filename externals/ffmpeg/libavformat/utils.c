                          ...
                          unsigned int offset, unsigned int max_probe_size)
{
    ...
    av_log(0, AV_LOG_INFO, "libavformat - av_probe_input_buffer 1\n");
    ...
    av_log(0, AV_LOG_INFO, "libavformat - av_probe_input_buffer 2\n");
    ...
    av_log(0, AV_LOG_INFO, "libavformat - av_probe_input_buffer 3\n");
    ...
    av_log(0, AV_LOG_INFO, "libavformat - av_probe_input_buffer 4\n");

    /* rewind. reuse probe buffer to avoid seeking */
    if ((ret = ffio_rewind_with_probe_data(pb, buf, pd.buf_size)) < 0)
        av_free(buf);

    av_log(0, AV_LOG_INFO, "libavformat - av_probe_input_buffer 5\n");
...
static int init_input(AVFormatContext *s, const char *filename, AVDictionary **options)
{
    ...
    av_log(0, AV_LOG_INFO, "libavformat - init_input\n");

    if (s->pb) {
        s->flags |= AVFMT_FLAG_CUSTOM_IO;
        if (!s->iformat)
        {
        	av_log(0, AV_LOG_INFO, "libavformat - init_input 1\n");
        ...
        av_log(0, AV_LOG_INFO, "libavformat - init_input 2\n");
        return 0;
    }

    av_log(0, AV_LOG_INFO, "libavformat - init_input 3\n");

    if ( (s->iformat && s->iformat->flags & AVFMT_NOFILE) ||
        (!s->iformat && (s->iformat = av_probe_input_format(&pd, 0))))
    {
    	av_log(0, AV_LOG_INFO, "libavformat - init_input 4\n");
    	return 0;
    }

    av_log(0, AV_LOG_INFO, "libavformat - init_input 5\n");

    if ((ret = avio_open2(&s->pb, filename, AVIO_FLAG_READ | s->avio_flags,
                          &s->interrupt_callback, options)) < 0)
    {
    	av_log(0, AV_LOG_INFO, "libavformat - init_input 6\n");
    	return ret;
    }

    av_log(0, AV_LOG_INFO, "libavformat - init_input 7\n");

    if (s->iformat)
    {
    	av_log(0, AV_LOG_INFO, "libavformat - init_input 8\n");
    	return 0;
    }

    av_log(0, AV_LOG_INFO, "libavformat - init_input 9\n");
...
int avformat_open_input(AVFormatContext **ps, const char *filename, AVInputFormat *fmt, AVDictionary **options)
{
    ...
    av_log(0, AV_LOG_INFO, "avformat_open_input 1\n");

    if (!s && !(s = avformat_alloc_context()))
    {
        av_log(0, AV_LOG_INFO, "avformat_open_input 2\n");
    	 return AVERROR(ENOMEM);
    }

    av_log(0, AV_LOG_INFO, "avformat_open_input 3\n");
    ...
    av_log(0, AV_LOG_INFO, "avformat_open_input 4\n");
    ...
    {
        av_log(0, AV_LOG_INFO, "avformat_open_input 5\n");
        goto fail;
    }
    ...
    {
    	av_log(0, AV_LOG_INFO, "avformat_open_input 6\n");
    	goto fail;
    }
    ...
    if (s->iformat->flags & AVFMT_NEEDNUMBER) {
        if (!av_filename_number_test(filename)) {
        	...
        	av_log(0, AV_LOG_INFO, "avformat_open_input 6.1 - error\n");
    ...
    av_log(0, AV_LOG_INFO, "avformat_open_input 7\n");
    ...
    av_log(0, AV_LOG_INFO, "avformat_open_input 8\n");
    ...
    av_log(0, AV_LOG_INFO, "avformat_open_input 9\n");
    ...
    av_log(0, AV_LOG_INFO, "avformat_open_input 10\n");
...
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options)
{
    ...
    av_log(ic, AV_LOG_DEBUG, "avformat_find_stream_info - Number of streams is %d\n", orig_nb_streams);
    ...
    for(i=0;i<ic->nb_streams;i++) {
        ...
        av_log(ic, AV_LOG_DEBUG, "avformat_find_stream_info - Iterating stream: %d, type = %d, codec id = %d\n", i, (int)st->codec->codec_type, (int)st->codec->codec_id);
...
/*            if(!st->time_base.num)
    ...
    av_log(ic, AV_LOG_DEBUG, "avformat_find_stream_info - 1\n");
...
//                     av_log(NULL, AV_LOG_ERROR, "%f\n", dts);
    ...
    av_log(ic, AV_LOG_DEBUG, "avformat_find_stream_info - 2\n");
    ...
    av_log(ic, AV_LOG_DEBUG, "avformat_find_stream_info - 3\n");
    ...
    av_log(ic, AV_LOG_DEBUG, "avformat_find_stream_info - 4\n");
    ...
    av_log(ic, AV_LOG_DEBUG, "avformat_find_stream_info - 5\n");
