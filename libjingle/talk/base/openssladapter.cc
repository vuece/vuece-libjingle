...
//NOTE: JJ - DO NOT move the position of openssladapter.h, it MUST BE THE FIRST header
//otherwise compile won't work on windows OS!
...
//    LOG(LS_VERBOSE) << "OpenSSLAdapter.cc::socket_read";
...
OpenSSLAdapter::StartSSL(const char* hostname, bool restartable) {
  ...
  if (int err = BeginSSL()) {
    Error("OpenSSLAdapter::StartSSL - BeginSSL", err, false);
    return err;
  }
...
OpenSSLAdapter::BeginSSL() {
  LOG(LS_INFO) << "OpenSSLAdapter:BeginSSL - host name: " << ssl_host_name_;
...
//    LOG(LS_VERBOSE) << "OpenSSLAdapter::Recv";
...
OpenSSLAdapter::OnCloseEvent(AsyncSocket* socket, int err) {
	//Note: when there is no connection, the error code is 2
	//Connection dropped - 110
...
bool OpenSSLAdapter::VerifyServerName(SSL* ssl, const char* host,
  ...
  for (int i = 0; i < extension_count; ++i) {
    ...
    if (extension_nid == NID_subject_alt_name) {
      ...
      if (meth->it) {
		  //JJ - What the fuck is wrong here??
